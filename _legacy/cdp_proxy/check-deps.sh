...
function activePortFiles() {
  ...
  switch (process.platform) {
    case 'darwin':
      // AdsPower browser profiles
      const adspowerDir = path.join(home, 'Library/Application Support/adspower_global/cwd_global/source/cache');
      let adspowerPorts = [];
      try {
        const profiles = fs.readdirSync(adspowerDir).filter(d => !d.startsWith('.'));
        adspowerPorts = profiles.map(p => path.join(adspowerDir, p, 'DevToolsActivePort'));
      } catch (_) {}
      return [
        ...adspowerPorts,
