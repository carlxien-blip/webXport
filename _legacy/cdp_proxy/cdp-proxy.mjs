...
async function discoverChromePort() {
  ...
  // 3. chrome://inspect 的远程调试使用随机端口，尝试通过 lsof 发现
  try {
    const { execSync } = await import('node:child_process');
    const output = execSync("lsof -iTCP -sTCP:LISTEN -P -n 2>/dev/null | grep 'Google Chrome' | head -5", { encoding: 'utf-8', timeout: 5000 });
    const ports = [...output.matchAll(/:(\d+)\s/g)].map(m => parseInt(m[1])).filter(p => p > 1024);
    for (const port of ports) {
      try {
        const res = await fetch(`http://127.0.0.1:${port}/json/version`);
        if (res.ok) {
          console.log(`[CDP Proxy] 通过 lsof 发现 Chrome 调试端口: ${port}`);
          return { port, wsPath: null };
        }
      } catch { /* not a CDP port */ }
    }
  } catch { /* lsof failed */ }
